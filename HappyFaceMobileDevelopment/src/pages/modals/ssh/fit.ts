...
import * as Terminal from "./exterm.js";
...
export function proposeGeometry(term: any): IGeometry {
    ...
    const geometry = {
        cols: Math.floor(availableWidth / (term.element.parentElement.clientWidth / term.cols)), //(<any>term).renderer.dimensions.actualCellWidth),
        rows: Math.floor(availableHeight / (term.element.parentElement.clientHeight / term.rows)) //(<any>term).renderer.dimensions.actualCellHeight)
    };
    return geometry;
}
